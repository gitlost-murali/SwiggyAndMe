...
def datetime_obj(inp):
    return datetime.strptime(inp, '%Y-%m-%d %H:%M:%S')

# df=pd.read_json("myhistory.json", typ='series', dtype={"articleId": np.float64})

with open("myhistory.json",'r',encoding='utf8') as fh:
    data = fh.read()
...
df['restaurant_customer_distance'] = df['restaurant_customer_distance'].astype('float32')
df['restaurant_customer_distance'] = df['restaurant_customer_distance'].fillna(0)
...
df['order_restaurant_bill'] = df['order_restaurant_bill'].astype('float32')
df['order_restaurant_bill'] = df['order_restaurant_bill'].fillna(0)
...
df['delivery_time_in_seconds'] = df['delivery_time_in_seconds'].fillna('0')
df['delivery_time_in_hours']=df['delivery_time_in_seconds'].astype('int32')/3600.0
# df['delivery_time_in_hours']=df['delivery_time_in_seconds']/3600.0

total_orders = len(df)

df['order_time'] = df['order_time'].apply(datetime_obj)
...
app.layout = html.Div(children=[
    html.H2(children='Swiggy & Me !', style={'textAlign': 'center'}),

    html.H6(children='Visualizing Swiggy Order History - Murali Manohar', style={'textAlign': 'center'}),
    ...
    ], target='_blank', href='https://www.linkedin.com/in/murali-manohar/', style={
                    'height' : '36px',
                    'width' : '36px',
                    'float' : 'right',
    ...
    ], target='_blank', href='https://twitter.com/gitlostmurali/', style={
                    'height' : '36px',
                    'width' : '36px',
                    'float' : 'right',
    ...
    ], target='_blank', href='https://gitlost-murali.github.io/', style={
                    'height' : '36px',
                    'width' : '36px',
                    'float' : 'right',
    ...
    dcc.Markdown('''
---
> You ordered a total of **'''+ str(total_orders) +'''** times.
